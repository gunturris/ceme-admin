...
function list_statistic_withdraw_player(){
   ...
   // $result = my_query($query);
	...
	while($ey = my_fetch_array($result)){ 
        ...
        $detail_button = '<a href="index.php?com='.$_GET['com'].'&task=withdraw&subtask=total&pid='.$ey['ID'].'">Withdraw</a>';
...
function withdraw_tabs($player_id , $page){
     my_set_file_js(
		array(
    ...
    $navigasi = array(
		'<input class="submit-green" type="button" value="By Player" onclick="javascript:location.href=\'index.php?com='.$_GET['com'].'&task=withdraw&subtask=player\'"/>',
		'<input class="submit-green" type="button" value="By Dates" onclick="javascript:location.href=\'index.php?com='.$_GET['com'].'&task=withdraw&subtask=total\'"/>' 
	);
    ...
    switch($page){
        case "total"  :
            $view .= list_statistic_withdraw( $player['username'] );
        break;
        case "player"  :
            $view .= list_statistic_withdraw_player();
        break; 
        default:
            $view .= list_statistic_withdraw( 0 );
        break;
