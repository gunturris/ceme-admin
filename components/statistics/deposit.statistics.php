...
function list_statistic_deposit_player(){
   ...
   // $result = my_query($query);
	...
	while($ey = my_fetch_array($result)){ 
        ...
        $detail_button = '<a href="index.php?com='.$_GET['com'].'&task=deposit&subtask=total&pid='.$ey['ID'].'">Deposit</a>';
...
function deposit_tabs($player_id , $page){
     my_set_file_js(
		array(
    ...
    $navigasi = array(
		'<input class="submit-green" type="button" value="By Player" onclick="javascript:location.href=\'index.php?com='.$_GET['com'].'&task=deposit&subtask=player\'"/>',
		'<input class="submit-green" type="button" value="By Dates" onclick="javascript:location.href=\'index.php?com='.$_GET['com'].'&task=deposit&subtask=total\'"/>' 
	);
    ...
    switch($page){
        case "total"  :
            $view .= list_statistic_deposit( $player['username'] );
        break;
        case "player"  :
            $view .= list_statistic_deposit_player();
        break; 
        default:
            $view .= list_statistic_deposit( 0 );
        break;
